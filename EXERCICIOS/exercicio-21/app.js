...
const impares = randomNumbers.filter((item) => item % 2 !== 0);
...
const number = crazyNumbers.reduce((acc, item) => {
  if (item < 501) {
    acc += 1;
  }
  return acc;
...
const numerosElevados = numbers.map((item) => item ** 2);
...
const filterAge = tarantinoMovies.filter((item) => item.release < 2000);
...
const namesTvShow = tvShows.reduce((acc, item) => {
  return `${acc} 
  - Nomes: ${item.name}`;
});

const tvShow = tvShows.map(({ name }) => name);
console.log(tvShow);
